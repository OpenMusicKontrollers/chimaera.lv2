...
{
	int oct = 2;
	float bot = oct*12.f - 0.5 - (160 % 18 / 6.f);
	float ran = 160.f / 3.f;
	float val = bot + cev->x * ran;
	
	dict_t *dict = _dict_ref(handle, cev->sid);
	uint8_t chn = dict->chn;
	uint8_t key = dict->key;

	uint8_t bnd = (val-key) / ran * 0x1fff + 0x2000;
	uint8_t bnd_msb = bnd >> 7;
	uint8_t bnd_lsb = bnd & 0x7f;

	uint8_t cnt = cev->z * 0x3fff;
...
_midi_on(handle_t *handle, int64_t frames, const chimaera_event_t *cev)
{
	...
	dict_t *dict = _dict_ref(handle, 0);
	dict->sid = cev->sid;
	dict->chn = chn;
	dict->key = key;
